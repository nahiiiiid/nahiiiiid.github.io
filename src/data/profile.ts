export const profile = {
  handle: "nahiiiiid",
  name: "Nahid",
  designation: "Software Engineer | Researcher",
  headline: "Nahid Hasan Saikot — Software Engineer & Researcher",
  subHeadLine: "SWE",
  subtext:
    "Systems thinking in action. Algorithms optimized, web architectures engineered, and experiments executed with rigor. Code, design, and problem-solving aligned for scalable, reliable solutions. Calm precision guiding every system built, every logic path tested, every problem resolved.\n\n" + 
    "Developer-grade interfaces and systems. Scalable web engineering, algorithmic problem solving, strong architecture patterns. CPS/IoT security and behavioral trust modeling. Leakage-aware evaluation, reproducible pipelines, security-relevant metrics (ROC-AUC, FPR, TNR, Macro-F1).", 

  location: "Dhaka, BD",
  contact: {
    email: "mailto:nahidhasansaikot@gmail.com",
    whatsapp: "https://wa.me/8801516546810",
  },
 social: {
  github: "https://github.com/nahiiiiid",
  facebook: "https://facebook.com/nahiiiiiiiiiid",
  youtube: "https://youtube.com/@nahiiiiiiiiiid",

  x: "https://x.com/nahiiiiid",
  linkedin: "https://www.linkedin.com/in/your-linkedin-username/",
  leetcode: "https://leetcode.com/your-username/",
  codeforces: "https://codeforces.com/profile/your-username",
  kaggle: "https://kaggle.com/your-username",
  hackerrank: "https://www.hackerrank.com/your-username",
} as const,

  // aboutNarrative: [
  //   "I build software like a precision instrument: define the system boundary, model constraints, implement with correctness, and verify outcomes. ",
  //   "I design interfaces that feel like developer workstations — clean, fast, keyboard-friendly, and structured. On the backend, I prefer clear abstractions and measurable performance. I design interfaces that feel like developer workstations — clean, fast, keyboard-friendly, and structured. On the backend, I prefer clear abstractions and measurable performance",
  //   "On the research side, I focus on rigorous evaluation and leakage-aware experimentation, especially in CPS/IoT security and behavioral trust modeling.",
  //   "I build software like a precision instrument: define the system boundary, model constraints, implement with correctness, and verify outcomes.",
  //   "I design interfaces that feel like developer workstations — clean, fast, keyboard-friendly, and structured. On the backend, I prefer clear abstractions and measurable performance.",
  //   "On the research side, I focus on rigorous evaluation and leakage-aware experimentation, especially in CPS/IoT security and behavioral trust modeling.",
  // ],


aboutPosts: [
  {
    title: "What am I?",
    text:
      "I build software like a precision instrument: define the system boundary, model constraints, implement with correctness, and verify outcomes.\n\n" +
      "I build software like a precision instrument.\n" +
      "Define boundaries, model constraints, implement with correctness, verify outcomes.\n\n" +
      "Calm, structured, engineering-first execution — always.\n\n" +
      "I build software like a precision instrument: define the system boundary, model constraints, implement with correctness, and verify outcomes.\n\n" +
      "I design interfaces that feel like developer workstations — clean, fast, keyboard-friendly, and structured. On the backend, I prefer clear abstractions and measurable performance.",
  },
  {
    title: "What am I doing?",
    text:
      "Building developer-grade interfaces and systems.\n\n" +
      "Working on scalable web engineering, algorithmic problem solving, and strong architecture patterns.",
  },
  {
    title: "Research focus",
    text:
      "CPS/IoT security and behavioral trust modeling.\n\n" +
      "Leakage-aware evaluation, reproducible pipelines, and security-relevant metrics (ROC-AUC, FPR, TNR, Macro-F1).",
  },
] as const,


  badges: ["systems thinking", "research mindset", "keyboard-first"],
  stack: ["React", "TypeScript", "Vite", "Tailwind", "Node.js"],
} as const;
